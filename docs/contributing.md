...
| `leak-audit.py` | 一手对象对着一份实测基线泄漏：一手类**集合**不得扩大、**每主机的扇出**与**每主机字节**不得越界（绝对计数不作判据——同一份构建 9 次实测主机数 4–8、比值恒为 3）（`--list` 逐块读；读到的块数与报告自己计数的块数对不上就红——**读不到泄漏=绿**是内存门禁独有的失败方式，2026-09-24 就被树状报告骗过一次）。`--self-test` 之外还有 `--red-team`：拿真实抓取的报告（`scripts/leak-sample.txt`，已抹掉本机标识）逐条破坏，验证每条破坏都被正确拒绝、且「真的修好一类」必须放行 |
