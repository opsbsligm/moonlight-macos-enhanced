...
3. **24** -- the strategy and both unobserved status claims are gone. What remains is a
   ...
   Until then this row says what is unknown. The counters that would answer it exist and
   are collected -- arrived, sent, suppressed, and the CoreHID pair -- but only the in-stream
   HUD shows them, so the next step for this issue is an input section in the copyable
   report rather than another guess.
