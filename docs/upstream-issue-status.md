...
3. **24** -- the strategy and both unobserved status claims are gone. What remains is a
   ...
   Until then this row says what is unknown. The counters that would answer it are now in
   the copyable report rather than only in the in-stream HUD -- the `input` section carries
   arrived, sent, suppressed, the CoreHID pair and the packets per sender -- so what remains
   is the measurement itself, which needs a host and a mouse rather than another gate.
...
5. **19, 28, 29, 33, 35** -- nothing to build, and now an excuse less. The app builds a
   ...
   whether `_nvstream._tcp` is among them, an `input` block -- the mouse strategy in effect
   with the value stored for it, the CoreHID answers, the sender that last moved the host's
   pointer, and with `Input Diagnostics` on the per-sender packet counts -- the hosts in the
   store with their pair and online state, and the tail of its own log. A PIN, a password, a certificate-shaped blob, a MAC
