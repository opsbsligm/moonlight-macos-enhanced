...
- **CI 里没有 `leaks` 门禁**：**这条在 2026-09-24 当天作废**——`scripts/leak-audit.py` 现在就是门禁，
  形状按本节原来的建议落：一手类**集合**不得扩大 + 每类**实例数上限** + 总字节上限，而不是要求计数相等。
  上限取自本机同一份 Debug 探针构建连跑 5 次（每类 18/6 五次不变，总字节在 10,000–11,792 间动），
  所以每类上限写死、字节上限按最高值加一半余量取 17,700。
  **仍未做的**：CI runner 是全新 HOME、没有配置主机，因此那边一手图整体不泄漏，
  所以 CI 上的绿只担保「没有新的一类开始泄漏」，不担保本机的 18/6 已被复现；
  要在 runner 上取真实基线，得先在那台 runner 上跑一次 `--write-baseline` 并把它写进同一个 commit。
...
- **红证（`--red-team`）**：拿一份真实抓取的报告逐条破坏，看每破坏是否都被拒、且拒得有理：
  超每类上限 / 冒出没预算的类 / 超总字节上限 / 汇总行被截掉 / 块被删而汇总留着 / 真的修好了一类（这条必须**放行**并给 note）。
  6 条破坏 6 个正确答复；对着上面那份树状报告跑时，红证**拒绝运行**而不是给出假绿。
...
**红证证不了什么**：它证明读取器还在读，不证明上限数字是对的；也不证明 §5 的所有权修法可以做——
那条仍卡在「串流会话里 `app.host` 不为 nil」这条本地给不出的证据上。
