...
- **所以门禁判的是形状不是计数**：一手类集合不得扩大；**扇出**（每个泄漏主机带几个 app）不得超过实测比值 + 每主机余量
  ...
  主机数本身不受上限约束——那是 LAN 的数字。**没测到**：地板值 2,000 是给「一个主机都没发现」的 runner 预留的占位，
  本机给不出这种运行；那种运行下字节预算**不判**并明说不判，而「有 app 没主机」一律判红（没有环境可以甩锅）。
  要在 runner 上取真实基线，得在那台 runner 上跑 `--write-baseline`（只会抬高、不会压低）并写进同一个 commit。
