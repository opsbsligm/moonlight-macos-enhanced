...
public class LanguageManager: NSObject, ObservableObject {
  ...
  private let en: [String: String] = [
    ...
    "Shortcut reserved by gameplay keys": "Shift is a game action key (sprint, aim), so Moonlight would take Shift plus a movement key away from the game while you play. Add Control, Option or Command.",
    "Cancel": "Cancel",
  ...
  private let zhHans: [String: String] = [
    ...
    "Shortcut reserved by gameplay keys": "Shift 在游戏中本身就是动作键（奔跑、瞄准）。只用 Shift 加移动键时，Moonlight 会在游玩过程中把这个键从游戏里抢走。请再加上 Control、Option 或 Command。",
    "Cancel": "取消",
