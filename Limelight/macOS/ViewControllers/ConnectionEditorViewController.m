...
- (void)loadView {
    ...
    NSTextField *title = [NSTextField labelWithString:[NSString stringWithFormat:MLString(@"Connection options - %@", nil), self.host.displayName ?: @"-"]];
    ...
    NSTextField *defaultLabel = [NSTextField labelWithString:MLString(@"Default connection", nil)];
    ...
    NSTextField *currentLabel = [NSTextField labelWithString:MLString(@"This session's connection", nil)];
    ...
    addressCol.title = MLString(@"Address", nil);
    ...
    self.addField.placeholderString = MLString(@"Add address (IP / hostname / IPv6, port optional)", nil);
    [addStack addArrangedSubview:self.addField];

    NSButton *addButton = [NSButton buttonWithTitle:MLString(@"Add", nil) target:self action:@selector(addEndpoint)];
    [addStack addArrangedSubview:addButton];

    self.removeButton = [NSButton buttonWithTitle:MLString(@"Remove", nil) target:self action:@selector(removeSelectedEndpoint)];
    ...
    self.testAllButton = [NSButton buttonWithTitle:MLString(@"Test all", nil) target:self action:@selector(testAllEndpoints)];
    [actions addArrangedSubview:self.testAllButton];

    NSButton *closeButton = [NSButton buttonWithTitle:MLString(@"Close", nil) target:self action:@selector(closeSheet)];
...
- (void)reloadMethodPopups {
    ...
    [self.defaultPopup addItemWithTitle:MLString(@"No default (automatic)", nil)];
    self.defaultPopup.lastItem.representedObject = @"__none__";

    [self.defaultPopup addItemWithTitle:MLString(@"Automatic", nil)];
    self.defaultPopup.lastItem.representedObject = @"Auto";

    [self.currentPopup addItemWithTitle:MLString(@"Automatic", nil)];
...
- (NSString *)sourceLabelForAddress:(NSString *)addr manualSet:(NSSet *)manualSet {
    ...
    if (self.host.address && [addr isEqualToString:self.host.address]) {
        return MLString(@"Manual address", nil);
    }
    return @"";
}
...
- (NSString *)statusTextForAddress:(NSString *)addr {
    ...
    if (effectiveState == 1) {
        if (latency && latency.intValue >= 0) {
            return [NSString stringWithFormat:MLString(@"Online (%d ms)", nil), latency.intValue];
        }
        return MLString(@"Online", nil);
    }
    if (effectiveState == 0) {
        return MLString(@"Offline", nil);
    }
...
- (void)updateSummaryLabels {
    ...
    self.statusLabel.stringValue = [NSString stringWithFormat:MLString(@"Host status: %@", nil), anyOnline ? MLString(@"Online", nil) : MLString(@"Offline", nil)];
    NSString *autoAddr = self.host.activeAddress ?: @"-";
    self.autoLabel.stringValue = [NSString stringWithFormat:MLString(@"Automatic choice: %@", nil), autoAddr];
}
...
- (void)handleDoubleClick:(id)sender {
    ...
    alert.messageText = MLString(@"Edit connection options", nil);
    alert.informativeText = MLString(@"Saving a new address replaces the current connection options.", nil);

    NSTextField *inputField = [[NSTextField alloc] initWithFrame:NSMakeRect(0, 0, 320, 24)];
    inputField.stringValue = oldAddr;
    alert.accessoryView = inputField;
    [alert addButtonWithTitle:MLString(@"Save", nil)];
